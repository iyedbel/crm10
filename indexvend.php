...
<option value="Ksar Chellala">Ksar Chell
                ...
                <label for="filterMonth">Filtrer par mois de création:</label>
<input type="month" id="filterMonth" class="px-4 py-2 border rounded-md">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    filterwilaya.addEventListener("change", function () {
        ...
        rows.forEach(row => {
            const wilaya = row.querySelector(".wilaya-column").textContent.trim(); // Utilisez la classe correcte
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        filterMonth.addEventListener("change", function () {
            ...
            rows.forEach(row => {
                const dateCreation = row.querySelector(".date-creation-column").textContent.trim(); // Assurez-vous d'utiliser la classe correcte
                const month = dateCreation.split('-')[1]; // Obtenez le mois à partir de la date de création
                const selectedMonth = selectedDate.split('-')[1]; // Obtenez le mois à partir de la date sélectionnée
                if (selectedDate === "all" || month === selectedMonth) {
                    row.style.display = "";
                } else {
                    row.style.display = "none";
                }
